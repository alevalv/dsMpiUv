...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < ROUNDS; i++)
    {
        ...
        if (taskid != MASTER)
        {
            ...
            /* Insertar la funcion bloqueante de envio de datos que se encargara de */
            /* enviar el valor en la variable homepi al proceso MASTER y cuyo tag esta */
            /* dado en la variable mtype. El valor de retorno de la funcion de envio  */
            /* de datos se guarda en la variable rc */
        ...
        else
        {
            ...
            {
                rc = MPI_Recv(&pirecv, 1, MPI_DOUBLE, n, mtype, MPI_COMM_WORLD, &status);
                /* Insertar la funcion bloqueante de recepcion de datos que se encargara de */
                /* recibir el valor de pi calculado por otro proceso en la variable pirecv. */
                /* Note que el MASTER es quien recibe el dato y lo debe recibir desde */
                /* cualquier otro proceso, MPI_ANY_SOURCE. El tag esta dado en la variable */
